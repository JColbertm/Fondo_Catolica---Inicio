<?php 
	class ClaseUsuario
	{
		...
		public static function encontrar_por_id($id){
			$resultado= execSqlA("SELECT a.*,b.*,c.*, d.idAfiliacion,d.fecha FROM usuario a, datos_secundario b, historial_sueldo c, form_afiliacion d where  a.idUsuario=b.idUsuario and a.idUsuario=c.idUsuario and a.idUsuario=$id and d.idUsuario=a.idUsuario and a.estado=1 order by c.idHistorial_sueldo asc limit 1");
			$objeto_array=array();
			while ($row = mysqli_fetch_array($resultado)) {
				$objeto_array[]=self::instanciacion($row);
			}
		...
		public static function registrar_afiliado($fecha,$carnet,$nombre,$apellido,$direccion,$telefono,$celular,$departamento,$interno,$correo,$totGanado,$liquido,$antiguedad,$aporte,$mes,$creador){
			...
			if($antiguedad>=$parametro['condicion'])
			{
				...
				$user = substr($nombre,0,1).'.'.$ap1.'.'.substr($ap2,0,1);
				$pass = '1234';
				...
				$result= insertA('usuario', array('ci','nombre','nombre2', 'apellido_p', 'apellido_m', 'idTipo_usuario', 'idUsuarioCreador','password','nombre_usuario','estado'), array(2,2,2,2,2,2,2,2,2,2) , array($carnet,$nom1,$nom2,$ap1,$ap2,1,1,$encrypted,$user,1));
				...
				$result3= insertA('form_afiliacion', array('idUsuario','fecha','hora','idUsuarioCreador'), array(2,2,2,2) , array($var,$fecha,date("g:i A"),1));//$_SESSION['id_en']

				$result4= insertA('historial_sueldo', array('cantidad_sueldo','monto_aporte','idMes','idUsuario','liquido','antiguedad'), array(2,2,2,2,2,2) , array($totGanado,$aporte,$mes,$var,$liquido,$antiguedad));
				...
				if ($result && $result2 && $result3 && $result4){		
					...
					$response['total']=$totGanado;
					$response['liquido']=$liquido;
					$response['aporte']=$aporte;
					$response['fecha']=$fecha;

					
				}else{	$response['resp']=0;}
			}
			else{$response['resp']=2;}
			return $response;

		}

		public static function modificar_afiliado($fecha,$carnet,$nombre,$apellido,$direccion,$telefono,$celular,$departamento,$interno,$correo,$totGanado,$liquido,$antiguedad,$aporte,$mes,$creador,$id){
			$parametro = RecuperarIdItemA('parametros',array('parametro'),array('antiguedad'));
			...
			if($antiguedad>=$parametro['condicion'])
			{
				...
				$result = updateA('usuario',array('ci','nombre','nombre2', 'apellido_p', 'apellido_m'),array(2,2,2,2,2),array($carnet,$nom1,$nom2,$ap1,$ap2),'idUsuario',$id);

				$result2 = updateA('datos_secundario',array('idUsuario','direccion','telefono','celular','departamento','correos','interno'),array(2,2,2,2,2,2,2),array($id, $direccion,$telefono,$celular,$departamento,$correo,$interno),'idUsuario',$id);

				$result3 = updateA('historial_sueldo',array('cantidad_sueldo','monto_aporte','idMes','idUsuario','liquido','antiguedad'),array(2,2,2,2,2,2),array($totGanado,$aporte,$mes,$id,$liquido,$antiguedad),'idUsuario',$id);
